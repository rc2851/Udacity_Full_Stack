...
CONF_BY_CITY = endpoints.ResourceContainer(
    message_types.VoidMessage,
...
# - - - Session objects - - - - - - - - - - - - - - - - -
    ...
    def _createSessionObject(self, request):
        ...
        # Set session speaker, and cache featured speaker
        sessions = Session.query(Session.speaker == session.speaker).fetch()
        if len(sessions) > 1:
            # If a speaker has more than 1 session in conference, then that speaker is the featured speaker.
    ...
    def getConferenceSessions(self, request):
        """Return all sessions associated with a conference"""
...
# - - - Wishlist - - - - - - - - - - - - - - - - -
    ...
    def addSessionToWishlist(self, request):
        ...
        if wishlist_session:
            raise endpoints.BadRequestException('Session is already in wish list.')
    ...
    def getSessionsInWishlist(self, request):
        ...
        if hasattr(request, 'websafeConferenceKey'):
            if request.websafeConferenceKey:
                wls = wls.filter(WishlistSession.sessionConferenceKey == request.websafeConferenceKey)
        sessions = ndb.get_multi([ndb.Key(urlsafe=wishList.sessionKey) for wishList in wls])

        return SessionForms(items=[self._copySessionToForm(session) for session in sessions])

# - - - 2 additional queries - - - - - - - - - - - - - - - - - - - - - - - - -

    @endpoints.method(CONF_BY_CITY, ConferenceForms,
